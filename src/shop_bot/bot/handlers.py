...
async def show_main_menu(message: types.Message, edit_message: bool = False):
    ...
    try:
        keyboard = keyboards.create_main_menu_keyboard(user_keys, trial_available, is_admin_flag, balance=balance)
    except Exception as e:
        logger.warning(f"Failed to create main menu keyboard: {e}")
        # Fallback without dynamic params if something breaks
...
def get_user_router() -> Router:
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        await callback.message.edit_text(final_text, reply_markup=keyboards.create_profile_menu_keyboard(balance=main_balance))
    ...
    async def help_handler(callback: types.CallbackQuery):
        await callback.answer()
        support_text = get_setting("support_text") or "Раздел поддержки."
        support_user = get_setting("support_user")
        has_external = bool(support_user)
        
        await callback.message.edit_text(
            support_text,
            reply_markup=keyboards.create_support_menu_keyboard(has_external=has_external)
        )
    ...
    async def show_instruction_handler(callback: types.CallbackQuery):
        ...
        text = get_setting("howto_intro_text") or "Выберите вашу платформу для инструкции по подключению VLESS:"
