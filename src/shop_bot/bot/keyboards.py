...
from shop_bot.data_manager.database import get_setting, get_button_configs
...
def create_dynamic_keyboard(menu_type: str, **kwargs) -> InlineKeyboardMarkup | None:
    buttons = get_button_configs(menu_type)
    if not buttons:
        return None
    ...
    # Pre-process kwargs to handle specific replacements like len(user_keys)
    if 'user_keys' in kwargs:
        kwargs['len(user_keys)'] = len(kwargs['user_keys'])
    
    for btn in buttons:
        if not btn['is_active']:
            continue
            
        # Special check for trial button: only show if trial_available and enabled
        if btn['button_id'] == 'trial':
            if not (kwargs.get('trial_available') and get_setting("trial_enabled") == "true"):
                continue
        
        # Special check for admin button
        if btn['button_id'] == 'admin':
        ...
        try:
            # Safe formatting
            # Handle {len(user_keys)} by simple replace if format fails or generally
            if '{len(user_keys)}' in text and 'user_keys' in kwargs:
                 text = text.replace('{len(user_keys)}', str(len(kwargs['user_keys'])))
            
            text = text.format(**kwargs)
        except Exception as e:
            # logger.warning(f"Failed to format button text '{text}': {e}")
            pass
    ...
    for r in sorted_rows:
        row_btns = rows[r]
        row_btns.sort(key=lambda x: x['column_position'])
        layout.append(len(row_btns))
...
def create_main_menu_keyboard(user_keys: list, trial_available: bool, is_admin: bool, balance: float = 0.0) -> InlineKeyboardMarkup:
    # Try dynamic first
    dynamic = create_dynamic_keyboard("main_menu", user_keys=user_keys, trial_available=trial_available, is_admin=is_admin, balance=balance)
    if dynamic:
        return dynamic
    ...
    if trial_available and get_setting("trial_enabled") == "true":
        btn_text = get_setting("btn_trial_text") or "🎁 Попробовать бесплатно"
        builder.button(text=btn_text, callback_data="get_trial")

    btn_profile_text = get_setting("btn_profile_text") or "👤 Мой профиль"
    builder.button(text=btn_profile_text, callback_data="show_profile")
    
    btn_my_keys_text = get_setting("btn_my_keys_text") or "🔑 Мои ключи"
    builder.button(text=f"{btn_my_keys_text} ({len(user_keys)})", callback_data="manage_keys")
    
    btn_referral_text = get_setting("btn_referral_text") or "🤝 Реферальная программа"
    builder.button(text=btn_referral_text, callback_data="show_referral_program")
    
    btn_support_text = get_setting("btn_support_text") or "🆘 Поддержка"
    builder.button(text=btn_support_text, callback_data="show_help")
    
    btn_about_text = get_setting("btn_about_text") or "ℹ️ О проекте"
    builder.button(text=btn_about_text, callback_data="show_about")
    
    btn_howto_text = get_setting("btn_howto_text") or "❓ Как использовать"
    builder.button(text=btn_howto_text, callback_data="howto_vless")
    
    # Speedtest button (optional in fallback, usually main menu has it if configured)
    btn_speed_text = get_setting("btn_speed_text") or "⚡ Скорость"
    builder.button(text=btn_speed_text, callback_data="user_speedtest_last")

    if is_admin:
        btn_admin_text = get_setting("btn_admin_text") or "⚙️ Админка"
        builder.button(text=btn_admin_text, callback_data="admin_menu")
...
def create_admin_menu_keyboard() -> InlineKeyboardMarkup:
    dynamic = create_dynamic_keyboard("admin_menu")
    if dynamic:
        return dynamic
...
def create_profile_menu_keyboard(balance: float = 0.0) -> InlineKeyboardMarkup:
    dynamic = create_dynamic_keyboard("profile_menu", balance=balance)
    if dynamic:
        return dynamic
    return create_profile_keyboard() # Fallback
...
def create_support_menu_keyboard(has_external: bool = False) -> InlineKeyboardMarkup:
    dynamic = create_dynamic_keyboard("support_menu", has_external=has_external)
    if dynamic:
        return dynamic
