...
from shop_bot.data_manager.database import (
    ...
    get_button_configs, get_button_config, create_button_config, update_button_config,
    delete_button_config, reorder_button_configs,
    get_user_payment_history, get_user_balance_history, get_subscription_stats, set_trial_status,
    get_referral_count, get_host, get_all_hosts
)
...
ALL_SETTINGS_KEYS = [
    ...
    # Content Settings
    "main_menu_text",
    "howto_intro_text", "howto_android_text", "howto_ios_text", "howto_windows_text", "howto_linux_text",
    "btn_trial_text", "btn_profile_text", "btn_my_keys_text", "btn_buy_key_text",
    "btn_topup_text", "btn_referral_text", "btn_support_text", "btn_about_text",
    "btn_speed_text", "btn_howto_text", "btn_admin_text", "btn_back_to_menu_text"
]
...
def create_webhook_app(bot_controller_instance):
    ...
    def create_key_ajax_route():
        ...
        if mode == 'personal':
            try:
                ...
                return jsonify({"ok": False, "error": "user_id/email required for personal mode"}), 400
        else:
            # Gift mode
            ...
            attempt = 0
            while True:
                key_email = f"{base_local if attempt == 0 else base_local + '-' + str(attempt)}@{domain}"
                if not get_key_by_email(key_email):
                    break
                attempt += 1
            user_id = 0
        ...
        # notify user (only for personal)
        if mode == 'personal' and user_id != 0:
            try:
                bot = _bot_controller.get_bot_instance()
                if bot and new_id:
                    text = (
    ...
    def user_details_json(user_id):
        try:
            ...
            user_data = {
                ...
                'referral_code': f"ref_{user.get('telegram_id')}",
                'referral_count': get_referral_count(user_id),
                'referred_by': {'username': referrer.get('username'), 'telegram_id': referrer.get('telegram_id')} if referrer else None
            }

            # Subscriptions
            subs_stats = get_subscription_stats(user_id)
            keys = get_user_keys(user_id)
            subscriptions = []
            
            # Pre-fetch all hosts to avoid repeated DB calls
            all_hosts = get_all_hosts()
            hosts_map = {h['host_name']: h for h in all_hosts}
            
            # Group keys by host to batch API calls
            keys_by_host = {}
            for k in keys:
                hname = k.get('host_name')
                if hname:
                    keys_by_host.setdefault(hname, []).append(k)
            
            # Cache tokens: uuid -> token
            tokens_map = {}
            
            # Fetch tokens from hosts (synchronously for now, as py3xui is sync)
            for hname, host_keys in keys_by_host.items():
                host_info = hosts_map.get(hname)
                if not host_info: continue
                try:
                    # Attempt to login and fetch clients
                    ...
                    if inbound and inbound.settings and inbound.settings.clients:
                        for client in inbound.settings.clients:
                            # Try to extract token/subId
                            token = None
                            for attr in ("subId", "subscription", "sub_id", "subscriptionId", "subscription_token"):
                                if hasattr(client, attr):
                                    token = getattr(client, attr)
                                elif isinstance(client, dict):
                                    token = client.get(attr)
                                if token: break
                            
                            cid = getattr(client, "id", None)
                            if not cid and isinstance(client, dict): cid = client.get("id")
                            
                            if cid and token:
                                tokens_map[cid] = token
                except Exception as e:
                    logger.warning(f"Failed to fetch tokens from host {hname}: {e}")

            now = datetime.now()
            for k in keys:
                try:
                    exp_str = k.get('expiry_date')
                    if exp_str:
                    ...
                    status_text = "Истек" if is_expired else "Активен"
                    
                    # Construct host info and link
                    host_name = k.get('host_name') or 'Unknown'
                    key_email = k.get('key_email') or 'no-email'
                    host_email = f"{host_name} | {key_email}"
                    
                    sub_link = 'Ссылка недоступна'
                    host_info = hosts_map.get(host_name)
                    if host_info and host_info.get('host_url'):
                        try:
                            uuid = k.get('xui_client_uuid')
                            token = tokens_map.get(uuid)
                            
                            # Generate link using xui_api helper
                            sub_link = xui_api.get_subscription_link(
                                user_uuid=uuid,
                                host_url=host_info['host_url'],
                                host_name=host_name,
                                sub_token=token
                            ...
                            logger.error(f"Error generating link for key {k.get('key_id')}: {e}")
                            sub_link = 'Ошибка генерации ссылки'

                    subscriptions.append({
                        'key': sub_link,  # For backward compatibility if needed, or update template to use sub_link
                        'sub_link': sub_link,
                    ...
                    logger.error(f"Error processing key {k.get('key_id')} for user details: {e}")
                    pass

            # Payment History
            raw_payments = get_user_payment_history(user_id)
            payment_history = []
            for p in raw_payments:
                ...
                months_str = f" ({meta.get('months')} мес)" if meta.get('months') else ""

                payment_history.append({
                    'date': p.get('created_date'),
                    'plan': f"{method}{months_str}",
                    'amount': p.get('amount_rub')
                })

            # Balance History
            raw_balance = get_user_balance_history(user_id)
            balance_history = []
            for b in raw_balance:
                meta = {}
                if b.get('metadata'):
                    try: meta = json.loads(b['metadata'])
                    except: pass

                b_type = "Списание"
                if meta.get('action') == 'top_up':
                    b_type = "Пополнение"
                elif b.get('payment_method') == 'Balance':
                    b_type = "Оплата услуг"

                balance_history.append({
                    'date': b.get('created_date'),
                    'type': b_type,
                    'amount': b.get('amount_rub')
                })
            ...
            logger.error(f"Error in user_details_json for {user_id}: {e}", exc_info=True)
            return jsonify({'ok': False, 'error': str(e)}), 500

    @flask_app.route('/users/<int:user_id>/referrals.json')
    @login_required
    def user_referrals_json(user_id):
        refs = get_referrals_for_user(user_id)
        # Serialize datetime objects if any
        # transactions returns dicts, but dates might be strings or objects depending on sqlite adapter.
        # usually sqlite returns strings for timestamps if not parsed.
        # But get_referrals_for_user returns a list of dicts.
        return jsonify({'ok': True, 'items': refs})

    @flask_app.route('/users/<int:user_id>/toggle-trial', methods=['POST'])
    @login_required
    def toggle_trial_used_route(user_id):
        user = get_user(user_id)
        if not user:
            return jsonify({'ok': False, 'error': 'User not found'}), 404

        current = bool(user.get('trial_used'))
        new_status = not current
        success = set_trial_status(user_id, new_status)

        return jsonify({
            'ok': success,
            'trial_used': new_status,
            'message': 'Статус пробного периода изменен' if success else 'Ошибка при изменении статуса'
        })
    ...
    def send_user_message_route(user_id):
        ...
        try:
            ...
            if bot:
                ...
                return jsonify({'ok': False, 'error': 'Bot instance not found'}), 500
        except Exception as e:
            return jsonify({'ok': False, 'error': str(e)}), 500
    ...
    def button_constructor_page():
        common_data = get_common_template_data()
        return render_template('button_constructor.html', **common_data)

    @flask_app.route('/api/button-configs/<menu_type>')
    @login_required
    def get_button_configs_api(menu_type):
        try:
            # Include inactive buttons for the constructor
            buttons = get_button_configs(menu_type, include_inactive=True)
            return jsonify({'success': True, 'data': buttons})
        except Exception as e:
    ...
    def update_button_config_api(button_id):
        try:
            ...
            if success:
                ...
                return jsonify({'success': False, 'error': 'Failed to update button config'}), 500
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)}), 500
    ...
    def delete_button_config_api(button_id):
        try:
            ...
            if success:
                ...
                return jsonify({'success': False, 'error': 'Failed to delete button config'}), 500
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)}), 500
    ...
    def reorder_button_configs_api(menu_type):
        try:
            data = request.json
            if 'button_orders' not in data:
                return jsonify({'success': False, 'error': 'Missing button_orders'}), 400

            success = reorder_button_configs(menu_type, data['button_orders'])
